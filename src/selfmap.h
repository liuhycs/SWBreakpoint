...
class selfmap {
  public:
    ...
    /// Check whether an address is inside our library itself.
    bool isCurrentLibrary(void* pcaddr, void** offset = NULL) {
      if(offset != NULL){
        *offset = (void*)((intptr_t)pcaddr - (intptr_t)_currentStart);
      }
      return ((pcaddr >= _currentStart) && (pcaddr <= _currentEnd));
    }
    ...
    void getTextRegions() {
      for(const auto& entry : _mappings) {
        const mapping& m = entry.second;
        if(m.isText()) {
          if(m.getFile().find("/libbreakpoint") != std::string::npos) {
          //if(m.getFile().find("/mylibrary") != std::string::npos) {
            _currentStart = (void*)m.getBase();
            _currentEnd = (void*)m.getLimit();
